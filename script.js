...
function generateLinks(rawQuery) {
  ...
  linkStatus.textContent = `Links ready for "${query}".`;
...
function updateSellThroughRate() {
  ...
  rateValue.textContent = `${rate.toFixed(rate >= 10 ? 0 : 1)}%`;
  ...
  if (rate > 100) {
    ...
    verdict.textContent = "Okay";
    verdict.className = "verdict okay";
  } else {
    verdict.textContent = "Slow / saturated";
